...
import { ContactUsTemplateKey } from "../../types/form";

type TemplateOption = {
  key: ContactUsTemplateKey;
  name: string;
  Component: React.ComponentType<any>;
};

type Props = {
  value: ContactUsTemplateKey | null;
  onChange: (val: ContactUsTemplateKey) => void;
  onNext: () => void;
  onBack: () => void;
...
export default function StepContactTemplate({ value, onChange, onNext, onBack, templates, disabled, dummyData }: Props) {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onNext(); }}>
      <h2 className="text-2xl font-bold mb-6">Choose a Contact Us Template</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {templates.map(({ key, name, Component }) => (
          <button
            ...
            aria-label={`Select ${name} contact us template`}
          >
      ...
      <div className="mt-8 flex justify-between">
        ...
        <button type="submit" className="bg-blue-600 text-white px-6 py-2 rounded disabled:opacity-50" disabled={disabled}>Finish</button>
      </div>
    </form>
  );
} 